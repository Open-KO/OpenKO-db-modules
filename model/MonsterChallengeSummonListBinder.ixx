...
export module FullModel:MonsterChallengeSummonListBinder;

import FullModel:MonsterChallengeSummonList;
...
namespace model
{
	...
	public:
		...
		static void BindIndex(model::MonsterChallengeSummonList& m, nanodbc::result& result, short colIndex)
		{
			result.get_ref<int16_t>(colIndex, m.Index);
		}

		/// \brief Binds a result's column to Level
		static void BindLevel(model::MonsterChallengeSummonList& m, nanodbc::result& result, short colIndex)
		{
			result.get_ref<uint8_t>(colIndex, m.Level);
		}

		/// \brief Binds a result's column to Stage
		static void BindStage(model::MonsterChallengeSummonList& m, nanodbc::result& result, short colIndex)
		{
			result.get_ref<uint8_t>(colIndex, m.Stage);
		}

		/// \brief Binds a result's column to StageLevel
		static void BindStageLevel(model::MonsterChallengeSummonList& m, nanodbc::result& result, short colIndex)
		{
			result.get_ref<uint8_t>(colIndex, m.StageLevel);
		}

		/// \brief Binds a result's column to Time
		static void BindTime(model::MonsterChallengeSummonList& m, nanodbc::result& result, short colIndex)
		{
			result.get_ref<int16_t>(colIndex, m.Time);
		}

		/// \brief Binds a result's column to MonsterId
		static void BindMonsterId(model::MonsterChallengeSummonList& m, nanodbc::result& result, short colIndex)
		{
			result.get_ref<int16_t>(colIndex, m.MonsterId);
		}

		/// \brief Binds a result's column to Count
		static void BindCount(model::MonsterChallengeSummonList& m, nanodbc::result& result, short colIndex)
		{
			result.get_ref<int16_t>(colIndex, m.Count);
		}

		/// \brief Binds a result's column to PosX
		static void BindPosX(model::MonsterChallengeSummonList& m, nanodbc::result& result, short colIndex)
		{
			result.get_ref<int16_t>(colIndex, m.PosX);
		}

		/// \brief Binds a result's column to PosZ
		static void BindPosZ(model::MonsterChallengeSummonList& m, nanodbc::result& result, short colIndex)
		{
			result.get_ref<int16_t>(colIndex, m.PosZ);
		}

		/// \brief Binds a result's column to Range
		static void BindRange(model::MonsterChallengeSummonList& m, nanodbc::result& result, short colIndex)
		{
