...
export module Binder:WebItemMall;

import Model;

namespace binder
{
	/// \brief generated nanodbc column binder for model::BinderWebItemMall
	...
	public:
		...
		static void BindAccountId(model::WebItemMall& m, const nanodbc::result& result, short colIndex)
		{
			result.get_ref<std::string>(colIndex, m.AccountId);
		}

		/// \brief Binds a result's column to CharId
		static void BindCharId(model::WebItemMall& m, const nanodbc::result& result, short colIndex)
		{
			result.get_ref<std::string>(colIndex, m.CharId);
		}

		/// \brief Binds a result's column to ServerId
		static void BindServerId(model::WebItemMall& m, const nanodbc::result& result, short colIndex)
		{
			result.get_ref<int16_t>(colIndex, m.ServerId);
		}

		/// \brief Binds a result's column to ItemId
		static void BindItemId(model::WebItemMall& m, const nanodbc::result& result, short colIndex)
		{
			result.get_ref<int32_t>(colIndex, m.ItemId);
		}

		/// \brief Binds a result's column to ItemCount
		static void BindItemCount(model::WebItemMall& m, const nanodbc::result& result, short colIndex)
		{
			result.get_ref<int16_t>(colIndex, m.ItemCount);
		}

		/// \brief Binds a result's column to BuyTime
		static void BindBuyTime(model::WebItemMall& m, const nanodbc::result& result, short colIndex)
		{
			result.get_ref<std::time_t>(colIndex, m.BuyTime);
		}

		/// \brief Binds a result's column to ImgFileName
		static void BindImgFileName(model::WebItemMall& m, const nanodbc::result& result, short colIndex)
		{
			m.ImgFileName = result.get<std::string>(colIndex);
		}

		/// \brief Binds a result's column to ItemName
		static void BindItemName(model::WebItemMall& m, const nanodbc::result& result, short colIndex)
		{
			m.ItemName = result.get<std::string>(colIndex);
		}

		/// \brief Binds a result's column to Price
		static void BindPrice(model::WebItemMall& m, const nanodbc::result& result, short colIndex)
		{
			m.Price = result.get<int32_t>(colIndex);
		}

		/// \brief Binds a result's column to PayType
		static void BindPayType(model::WebItemMall& m, const nanodbc::result& result, short colIndex)
		{
