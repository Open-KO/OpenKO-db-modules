...
export module Binder:MagicType5;

import Model;

namespace binder
{
	/// \brief generated nanodbc column binder for model::BinderMagicType5
	...
	public:
		...
		static void BindMagicNumber(model::MagicType5& m, const nanodbc::result& result, short colIndex)
		{
			result.get_ref<int32_t>(colIndex, m.MagicNumber);
		}

		/// \brief Binds a result's column to Name
		static void BindName(model::MagicType5& m, const nanodbc::result& result, short colIndex)
		{
			m.Name = result.get<std::string>(colIndex);
		}

		/// \brief Binds a result's column to Description
		static void BindDescription(model::MagicType5& m, const nanodbc::result& result, short colIndex)
		{
			m.Description = result.get<std::string>(colIndex);
		}

		/// \brief Binds a result's column to Type
		static void BindType(model::MagicType5& m, const nanodbc::result& result, short colIndex)
		{
			result.get_ref<int16_t>(colIndex, m.Type);
		}

		/// \brief Binds a result's column to ExpRecover
		static void BindExpRecover(model::MagicType5& m, const nanodbc::result& result, short colIndex)
		{
			result.get_ref<int16_t>(colIndex, m.ExpRecover);
		}

		/// \brief Binds a result's column to NeedStone
		static void BindNeedStone(model::MagicType5& m, const nanodbc::result& result, short colIndex)
		{
