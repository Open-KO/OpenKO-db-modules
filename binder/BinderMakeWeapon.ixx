module;

#include <nanodbc/nanodbc.h>
#include <string>
#include <unordered_map>

export module Binder:MakeWeapon;

import Model;

namespace binder
{
	/// \brief generated nanodbc column binder for model::BinderMakeWeapon
	export class MakeWeaponBinder
	{
	/// \publicsection
	public:
		typedef void (*BindColumnFunction_t)(model::MakeWeapon& m, const nanodbc::result& result, short colIndex);

		using BindingsMapType = std::unordered_map<std::string, BindColumnFunction_t>;

		/// \brief Returns the binding function associated with the column name
		static const BindingsMapType& GetColumnBindings()
		{
			static const BindingsMapType bindingsMap =
			{
				{"byLevel", &MakeWeaponBinder::BindLevel},
				{"sClass_1", &MakeWeaponBinder::BindClass1},
				{"sClass_2", &MakeWeaponBinder::BindClass2},
				{"sClass_3", &MakeWeaponBinder::BindClass3},
				{"sClass_4", &MakeWeaponBinder::BindClass4},
				{"sClass_5", &MakeWeaponBinder::BindClass5},
				{"sClass_6", &MakeWeaponBinder::BindClass6},
				{"sClass_7", &MakeWeaponBinder::BindClass7},
				{"sClass_8", &MakeWeaponBinder::BindClass8},
				{"sClass_9", &MakeWeaponBinder::BindClass9},
				{"sClass_10", &MakeWeaponBinder::BindClass10},
				{"sClass_11", &MakeWeaponBinder::BindClass11},
				{"sClass_12", &MakeWeaponBinder::BindClass12}
			};
			return bindingsMap;
		}

		/// \brief Binds a result's column to Level
		static void BindLevel(model::MakeWeapon& m, const nanodbc::result& result, short colIndex)
		{
			result.get_ref<int16_t>(colIndex, m.Level);
		}

		/// \brief Binds a result's column to Class1
		static void BindClass1(model::MakeWeapon& m, const nanodbc::result& result, short colIndex)
		{
			m.Class1 = result.get<int16_t>(colIndex);
		}

		/// \brief Binds a result's column to Class2
		static void BindClass2(model::MakeWeapon& m, const nanodbc::result& result, short colIndex)
		{
			m.Class2 = result.get<int16_t>(colIndex);
		}

		/// \brief Binds a result's column to Class3
		static void BindClass3(model::MakeWeapon& m, const nanodbc::result& result, short colIndex)
		{
			m.Class3 = result.get<int16_t>(colIndex);
		}

		/// \brief Binds a result's column to Class4
		static void BindClass4(model::MakeWeapon& m, const nanodbc::result& result, short colIndex)
		{
			m.Class4 = result.get<int16_t>(colIndex);
		}

		/// \brief Binds a result's column to Class5
		static void BindClass5(model::MakeWeapon& m, const nanodbc::result& result, short colIndex)
		{
			m.Class5 = result.get<int16_t>(colIndex);
		}

		/// \brief Binds a result's column to Class6
		static void BindClass6(model::MakeWeapon& m, const nanodbc::result& result, short colIndex)
		{
			m.Class6 = result.get<int16_t>(colIndex);
		}

		/// \brief Binds a result's column to Class7
		static void BindClass7(model::MakeWeapon& m, const nanodbc::result& result, short colIndex)
		{
			m.Class7 = result.get<int16_t>(colIndex);
		}

		/// \brief Binds a result's column to Class8
		static void BindClass8(model::MakeWeapon& m, const nanodbc::result& result, short colIndex)
		{
			m.Class8 = result.get<int16_t>(colIndex);
		}

		/// \brief Binds a result's column to Class9
		static void BindClass9(model::MakeWeapon& m, const nanodbc::result& result, short colIndex)
		{
			m.Class9 = result.get<int16_t>(colIndex);
		}

		/// \brief Binds a result's column to Class10
		static void BindClass10(model::MakeWeapon& m, const nanodbc::result& result, short colIndex)
		{
			m.Class10 = result.get<int16_t>(colIndex);
		}

		/// \brief Binds a result's column to Class11
		static void BindClass11(model::MakeWeapon& m, const nanodbc::result& result, short colIndex)
		{
			m.Class11 = result.get<int16_t>(colIndex);
		}

		/// \brief Binds a result's column to Class12
		static void BindClass12(model::MakeWeapon& m, const nanodbc::result& result, short colIndex)
		{
			m.Class12 = result.get<int16_t>(colIndex);
		}

	};
}
